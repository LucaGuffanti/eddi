...
from clementi_table import CLEMENTI_TABLE
import csv

class ClementiWaveFunction():
    ...
    def __init__(self, atomic_number: int, verbose: bool = False):
        ...
        self.electron_configurations = [el for el in CLEMENTI_TABLE.keys() if el[0] == atomic_number]
    ...
    def print_normalization_constants(self):
        ...
        with open(f'output/normalizations/normalization_constants_clementi_{self.atomic_number}.csv', mode='w', newline='') as file:
    ...
    def construct_wavefunction(self, electron_configuration: str) -> tuple:
        ...
        if self.verbose:
            print("=== Constructing wavefunction for :", electron_configuration, "===")


        n = electron_configuration[2][0]
        l = electron_configuration[2][1]

        single_orbitals = electron_configuration[1].split(' ')
        orbital = next((orb for orb in single_orbitals if orb.startswith(f"{n}{l}")), None)
        print(orbital)
        n_e = int(orbital[2:]) if orbital else 0
        if n_e == 0:
            print("No electrons in this level")
            return (None, None)
        
        n_star = SLATER_PRINCIPAL_QUANTUM_NUMBER[int(electron_configuration[1][0])]
        zeta = CLEMENTI_TABLE[electron_configuration]  
        ...
        if self.verbose:
            ...
            print(n_constant, n_e, n_star, zeta)
    ...
    def print_density_formula(self):
        ...
        with open(f'output/density_formulas_clementi.txt', 'a') as f:
            f.write(f"eddi_real_t eddi_{self.atomic_number}_clementi(eddi_real_t r, eddi_real_t /*theta*/, eddi_real_t /*phi*/)\n")
            f.write("{\n")
...
if __name__ == '__main__':
    for i in range(1,27):
        wf = ClementiWaveFunction(i, verbose=True)
        print("\n\nFinished\n\n")
